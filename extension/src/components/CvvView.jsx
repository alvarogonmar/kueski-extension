...
export default function CvvView({ token, pin, comercio, monto, quincenas, onDone }) {
  ...
  useEffect(() => {
    const generar = async () => {
      try {
        const data = await tokensAPI.generar(token, pin, comercio?.id || 1, monto, quincenas)
        setCvv(data.cvv)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--kueski-text-muted)', marginBottom: 8 }}>
          TU CVV VIRTUAL
        </div>
        <div style={{ fontSize: 48, fontWeight: 800, letterSpacing: 12, color: 'var(--kueski-blue)', marginBottom: 8 }}>
          {cvv}
        </div>
        <div style={{ fontSize: 13, color: segundos > 30 ? 'var(--kueski-primary)' : 'var(--kueski-danger)', fontWeight: 600 }}>
      ...
      <div style={{
        ...
        padding: '14px 16px', border: '1px solid var(--kueski-border)',
        fontSize: 12, color: 'var(--kueski-text-muted)', lineHeight: 1.6,
      }}>
      ...
      <button className="btn-secondary" onClick={copiarCvv}>
        {copiado ? '✅ Copiado!' : '📋 Copiar CVV'}
      </button>
